function [CurrX,FLG] = ImpedanceMatrixCalculation(CurrX,freq,FLG,envir,geom,exci)
...
for I =1:N  % 211 FOR I=1 TO N  (to 336)
    ...
    % Sa = segment length
    F4x = sign(Cp(I,1))*Sa(I1);          % 214 F4=SGN(C%(I,1))*S(I1)
    F5x = sign(Cp(I,2))*Sa(I2);          % 215 F5=SGN(C%(I,2))*S(I2)

    % 216 REM ----- R(M + 1/2) - R(M - 1/2) HAS COMPONENTS (T5,T6,T7)
    % CABG = Direction Cosines
    if Cp(I,1) == -Cp(I,2)              % 220
        dRm = [F4x F4x Sa(I1)]'.*CABG(I1,:)' + [F5x F5x Sa(I1)]'.*CABG(I2,:)';
    else
        dRm = F4x*CABG(I1,:)' + F5x*CABG(I2,:)';
    end
...
%             F8 = 0;                     % 235 F8=0
            if K < 0                    % 236 IF K<0 THEN 248
                Zmn = Beta_247_315Cmplx(I,J,K,J1,J2,F4,F5,F6,F7,0,dRm,Zmn,freq,geom); 
            else
...
[CurrX,FLG] = MatSolve(Zmn,freq,geom,exci);

SourceData(CurrX,exci);
